...
def get_analysis_files(analysis_pipeline, moh=False):
    '''
    (str, bool) -> list

    Returns a list of files generated by the analysis workflows 
    Pre-condition: The data is organized by case, workflow, workflow run and files
    or by donor, and datatype and files
    ...
    if moh:
        for donor in data:
            for datatype in data[donor]:
                L.extend(data[donor][datatype])
    else:
        for case_id in data:
            for workflow in data[case_id]:
                for workflow_id in data[case_id][workflow]:
                    L.extend(data[case_id][workflow][workflow_id])
